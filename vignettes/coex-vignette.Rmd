...
```{r, eval=FALSE}
cl <- calcAdjacency(cl, method = "wgcna", softPower = 6)
```
...
```{r, eval=FALSE}
calcModuleEigengenes <- function(cl, deepSplit=2, minClusterSize=10, pamStage=TRUE, ...){
        ...
        PCs <- WGCNA::moduleEigengenes(t(cl@normCounts), colors=modules, )
        ME <- PCs$eigengenes
        rownames(ME) <- rownames(colData(cl))
        ME <- t(ME)
        
}

pheatmap::pheatmap(mat = t(ME), annotation_col = as.data.frame(colData(cl)))
